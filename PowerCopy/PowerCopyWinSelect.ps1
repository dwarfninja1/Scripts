...
1.0.1 Updates:
-Add Folder Select Windows dialogs to pick folders for Source and Dest starting places
-save preferences function
-progressbarset
-stop Robocopy function

Version: 2.0.1 - 1/22/2015
Author: Grant Fraser
...
#default source and destination folders (if things get hinky)
$source = "C:\users"
$dest = "F:\users"
...
#source path select
$filepath = $source
$SourcePathSelected = FindFolders $filepath
$source = $SourcePathSelected 
Write-host "Source Path selected: $Source"                     

#dest path select
$filepath = ""
$DestPathSelected = $filepath
$DestPathSelected = FindFolders $filepath
$dest = $DestPathSelected
Write-host "Destination Path selected: $Dest" 
...
function robocopy {
    ...
    process {
        ...
        $run = robocopy.exe $InputSource.Text $InputTarget.Text $switchNP $switchR $switchW $switchS $switchE $switchB $switchSEC $switchCOPYALL $switchNOCOPY `
            ...
            $switchXL $switchIS $switchIT $switchXJ $switchXJD $switchXJF $switchL $switchX $switchV $switchTS $switchFP $switchBYTES $switchAddition $switchLogfile | ForEach-Object {
